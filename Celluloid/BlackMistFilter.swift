...
class BlackMistFilter: CIFilter {
    ...
    private static var blendKernel: CIBlendKernel? = {
        ...
        guard let url = Bundle.main.url(forResource: "default", withExtension: "metallib"),
            ...
            BlackMistFilter.logger.warning("Could not load default.metallib")
            return nil
        }

        do {
            let kernel = try CIBlendKernel(functionName: "blackMistBlend", fromMetalLibraryData: data)
            BlackMistFilter.logger.info("Metal kernel loaded successfully")
            return kernel
        } catch {
            BlackMistFilter.logger.error("Failed to create kernel: \(error)")
            return nil
        }
    }()
