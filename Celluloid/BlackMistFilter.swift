...
class BlackMistFilter: CIFilter {
    ...
    private static var blendKernel: CIBlendKernel? = {
        // Try to load from the app's default Metal library
        guard let url = Bundle.main.url(forResource: "default", withExtension: "metallib"),
              let data = try? Data(contentsOf: url) else {
            print("BlackMistFilter: Could not load default.metallib")
            return nil
        }

        do {
            let kernel = try CIBlendKernel(functionName: "blackMistBlend", fromMetalLibraryData: data)
            print("BlackMistFilter: Metal kernel loaded successfully")
            return kernel
        } catch {
            print("BlackMistFilter: Failed to create kernel: \(error)")
            return nil
        }
    }()
